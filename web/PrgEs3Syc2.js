...
function SaveSplan2(PLC,Parms,Plan)
{
	UpdateSyn2(PLC,Plan);
	var out="\tgoto INICIO\n";
	var i=0;
	...
	i=0;
	{
		...
		out+="STPtp,"+PlanGen.LCLSYCTSTSTP.toString()+"\n";
		if(PLC.McPlan)
		{
		...
		for(var j=0;j<TIM2STP.length;j++) 
		{
			if(Parms.MODEL.indexOf("GW4")!=-1 || Parms.MODEL.indexOf("GW")==-1)
			{
				...
				stp=parseInt(Plan.LCLSYCSEQSTP[stp]);
				sts1=parseInt(Plan.LCLSYCSTSSTP[stp])-1;
				temp2+=Plan.LCLSYCTOF;
				TIM2STP[j]=temp2%Plan.LCLSYCTCI;
				XEV=GetEvT(PLC,sts,sts1);
				temp2+=XEV;
			}
