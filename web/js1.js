...
function UpdateSizeOfStruct()
{
	...
	if(PHASEs.length!=(GlobalParms.Phases+GlobalParms.Virtual_Phases+GlobalParms.Groups_Phases))
	{
		for(var j=0;j<(GlobalParms.Phases+GlobalParms.Virtual_Phases+GlobalParms.Groups_Phases);j++)
		{
			...
			{
				PHASEs[j]=new Object();
				if(j>=(GlobalParms.Phases+GlobalParms.Virtual_Phases))
					PHASEs[j].FlagsWeb=1;
				else
					PHASEs[j].FlagsWeb=0;
				PHASEs[j].Numero=j;
...
function RcvPh_Conf(Datos)
{
	...
	while(j<Datos.length && j<PHASEs.length)
	{
		...
		else
		{
			...
			//PHASEs[PhN].LampTyp=parseInt("0"+Datos[j][10])&0x07;
		...
		else
		{
			...
			if(PhN<(GlobalParms.Phases+GlobalParms.Virtual_Phases+GlobalParms.Groups_Phases))
			{
				...
				if(Datos[j][7]!="")
				{
					...
					{
						temp[i]=temp[i].split(":");
						PHASEs[PhN].R2V[i]= new Object();
						PHASEs[PhN].R2V[i].Color=parseInt("0"+temp[i][0]);
						PHASEs[PhN].R2V[i].Tiempo=parseInt("0"+temp[i][1]);
						if(i>0)
							PHASEs[PhN].R2V[(i-1)].Tiempo-=PHASEs[PhN].R2V[i].Tiempo;
					}
					...
					{
						temp[i]=temp[i].split(":");
						PHASEs[PhN].V2R[i]= new Object();
						PHASEs[PhN].V2R[i].Color=parseInt("0"+temp[i][0]);
						PHASEs[PhN].V2R[i].Tiempo=parseInt("0"+temp[i][1]);
						if(i>0)
							PHASEs[PhN].V2R[(i-1)].Tiempo-=PHASEs[PhN].V2R[i].Tiempo;
					}
				}
				//-----------------------------------------
				if(Datos[j][8]!="")
				...
				//-----------------------------------------
				if(Datos[j][9]!="")
					PHASEs[PhN].LampTyp=parseInt("0"+Datos[j][9])&0x07;
				//-----------------------------------------
				//PHASEs[PhN].Sec= new Array();
			}
			j++;
...
function LogPHASEs()
{
	...
	for(var j=0;j<PHASEs.length;j++)
	{
		...
		{
			temp+=PHASEs[j].Sec[i].phase+":"+PHASEs[j].Sec[i].time+" ";
		}
		if(PHASEs[j].R2V)
		{
			temp+="\tR2V:"
		...
		for(var cft=0;cft<Datos.length;cft++)
		{
			...
			if(PhN<PHASEs.length)
			{
				...
				for(var i=0;i<Datos[cft].length;i++)
				{
					...
					PHASEs[PhN].Sec[i]= new Object();
					PHASEs[PhN].Sec[i].phase=parseInt("0"+Datos[cft][i][0]);
					PHASEs[PhN].Sec[i].time=parseInt("0"+Datos[cft][i][1]);
				}
			}
		}
	}
...
function RcvPHG(Datos)
{
	...
	{
		i=Datos.length;
		Datos=Datos.replace('(',''); //Replace troca os valores dentro de uma string
		Datos=Datos.replace(')',''); 
	}
	while(i!=Datos.length);
	Datos=Datos.split('\n');
	...
	if(GlobalParms.Groups_Phases)
	{
		for(var i=0;(i<Datos.length) && (i<GlobalParms.Groups_Phases);i++)
		{
			PhN=((GlobalParms.Phases+GlobalParms.Virtual_Phases)+parseInt("0"+Datos[i][0]));
			PHASEs[PhN].FlagsWeb=1;
			PHASEs[PhN].Numero="G"+i;
			PHASEs[PhN].Sec=Datos[i].slice(1,Datos[i].length);
			PHASEs[PhN].PLC=0;
		}
	}
}
...
function RcvDefIn(Datos)
{
	...
	for(i=0;i<Datos.length;i++)
	{
		...
		case "I/O":
			Datos[i][1]=Datos[i][1].split(",");
			Nin=parseInt("0"+Datos[i][1][0]);
			if(IOs.length>Nin)
			{
				IOs[Nin].Enable=parseInt("0"+Datos[i][1][1]); // INPUT
				IOs[Nin].Type=parseInt("0"+Datos[i][1][2]);
				if(IOs[Nin].Type==3)
					IOs[Nin].Type=(2|parseInt("0"+Datos[i][1][2]));
				IOs[Nin].neg=parseInt("0"+Datos[i][1][3]);
				IOs[Nin].Flank=parseInt("0"+Datos[i][1][4]);
				IOs[Nin].TimeOut=parseInt("0"+Datos[i][1][5]);
				IOs[Nin].FailSts=parseInt("0"+Datos[i][1][6]);
				IOs[Nin].Plcs=parseInt("0"+Datos[i][1][7]);
				IOs[Nin].Name=Datos[i][1][8];
			}
		break;
		default:
