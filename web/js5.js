	...
	for(var j=0;j<PLCs[PlcIdx].Phases.length;j++)
	{
		...
		if(i<PHASEs.length)
		{
			...
			//==-==-==-==-==-==-==-==-==-==-==-==-==-==-==-==-==
			{
				...
				{
					/*/----------------
					ErEr=   0x00000001,  //baja tension rojo
					ErER=   0x00000002,  //sobre tension en rojo
					ErCr=   0x00000004,  //baja corriente rojo
					ErCR=   0x00000008,  //sobre corriente rojo
					//----------------
					ErEy=   0x00000010,  //baja tension amarillo
					ErEY=   0x00000020,  //tension en amarillo
					ErCy=   0x00000040,  //baja corriente 
					ErCY=   0x00000080,  //sobre corriente
					//----------------
					ErEg=   0x00000100,  //baja tension verde
					ErEG=   0x00000200,  //tension en verde
					ErCg=   0x00000400,  //baja corriente 
					ErCG=   0x00000800,  //sobre corriente
					//----------------
					ErMiTR= 0x00001000,  // error minimo tiempo de rojo
					ErMaTR= 0x00002000,  // error maximo tiempo de rojo
					ErMiTY= 0x00004000,  // error minimo tiempo de amarillo
					ErMaTY= 0x00008000,  // error maximo tiempo de amarillo
					//- - - - - - - -
					ErMiTG= 0x00010000,  // error minimo tiempo de verde
					ErMaTG= 0x00020000,  // error maximo tiempo de verde
					//----------------
					ErLuxR= 0x00040000,  // error intensidad luminosa maxima rojo
					ErLuxr= 0x00080000,  // error intensidad luminosa minima rojo
					ErLuxY= 0x00100000,  // error intensidad luminosa maxima amarillo
					ErLuxy= 0x00200000,  // error intensidad luminosa minima amarillo
					ErLuxG= 0x00400000,  // error intensidad luminosa maxima verde
					ErLuxg= 0x00800000,  // error intensidad luminosa minima verde
					//----------------
					ErTLr=  0x01000000,  // Falta Total Red
					ErTLy=  0x02000000,  // Falta Total Yhellow
					ErTLg=  0x04000000,  // Falta Total Green
					//xxd=  0x08000000,  //
					//----------------
					ErTR=   0x10000000,  // error temperatura rojo
					ErTY=   0x20000000,  // error temperatura amarillo
					EtTG=   0x40000000,  // error temperatura verde
					//- - - - - - - -
					ErLnk=  0x80000000,  // Error de comunicacion 
					//----------------*/
					temp=	0x00000000;
					if(document.getElementById("PhEr"+i+"0").checked==1)temp|=0x01000000;
					if(document.getElementById("PhEr"+i+"1").checked==1)temp|=0x02000000;
					if(document.getElementById("PhEr"+i+"2").checked==1)temp|=0x04000000;
				...
				else
				{
					...
					*/
					...
					if(document.getElementById("PhEr"+i+"1").checked==1)temp|=0x00000002;
					if(document.getElementById("PhEr"+i+"2").checked==1)temp|=0x00000004;
